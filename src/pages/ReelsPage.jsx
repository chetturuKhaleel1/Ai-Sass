...
export default function ReelsPage() {
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            // 1. Start Export
            const startRes = await fetch(`${backend}/api/process/export/start`, {
                method: 'POST',
            ...
            const { exportId } = await startRes.json();

            // 2. Poll Progress
            const pollInterval = setInterval(async () => {
                try {
                    const statusRes = await fetch(`${backend}/api/process/export/status/${exportId}`);
                    ...
                    if (statusData.status === 'processing') {
                        ...
                        // 3. Download
                        window.location.href = `${backend}/api/process/export/download/${exportId}`;
                    ...
                    clearInterval(pollInterval);
                    setIsExporting(false);
                    alert("Export failed during processing.");
                }
            }, 1000);
        ...
        <div className="min-h-screen bg-black text-white flex flex-col md:flex-row overflow-hidden">
            ...
            <div className="w-full md:w-[380px] p-5 border-r border-white/10 flex flex-col gap-5 h-screen overflow-y-auto custom-scrollbar bg-zinc-950">
                ...
                <button
                    ...
                    className={`btn-primary-tech w-full py-3 flex items-center justify-center gap-2 mt-auto shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 transition-all text-sm ${isExporting ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                    {isExporting ? (
                        <>
            ...
            {isExporting && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center animate-fade-in">
                    <div className="bg-gradient-to-br from-zinc-900 to-black border border-white/10 rounded-2xl p-8 w-[90%] max-w-md shadow-2xl">
                        {/* Header */}
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-zinc-400">Burning captions into video...</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3 mb-6">
                            ...
                            <div className="relative h-3 bg-zinc-800 rounded-full overflow-hidden">
                                ...
                                {/* Progress Fill */}
                                <div
                                    className="absolute inset-0 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-full transition-all duration-500 ease-out shadow-lg shadow-indigo-500/50"
                                    style={{ width: `${exportProgress}%` }}
                                >
                                    {/* Shine Effect */}
                        ...
                        <div className="text-center text-sm text-zinc-500">
                            {exportProgress < 30 && "⚙️ Initializing FFmpeg..."}
                            {exportProgress >= 30 && exportProgress < 60 && "🎨 Applying caption styles..."}
                            {exportProgress >= 60 && exportProgress < 95 && "🎬 Rendering video frames..."}
                            {exportProgress >= 95 && exportProgress < 100 && "✨ Finalizing export..."}
                            {exportProgress === 100 && "✅ Download starting..."}
                        </div>
                    </div>
                </div>
            )}
